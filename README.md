...
Wiki Graph provides two complementary projections. Inheritance uses a centered circular Obsidian-style force graph with straight links; All Relations retains curved links and depth cues for dense data. Topology is a separate top-level view using a focused, progressively expandable tree with draggable instance-labeled nodes. Every distinct static instance remains separate, including repeated resolved or missing-definition types under one parent. Nodes with indexed children expose `+` / `-` controls, while a searchable root selector, Back, Focus selected, Collapse, Fit, and Reset view controls keep large repositories responsive. Each scene has a hard rendering budget; users can choose a deeper component as the root instead of expanding an entire VIP at once. Missing component definitions are retained as dashed warning nodes and link to their individual factory-create evidence. Selecting a topology node opens its factory-create or member-declaration evidence by default, while the selection panel provides a second direct entry to the class definition when one exists. The Related panel can filter by relation kind and search source, target, or location text.
