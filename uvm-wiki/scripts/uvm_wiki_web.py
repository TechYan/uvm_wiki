...
function init(){renderHeader();bind();renderRoles();renderArchRoots();renderArchitecture();renderWiki();renderTlm();renderCodeIndex();}
...
function bind(){document.querySelectorAll(".tab").forEach(button=>button.onclick=()=>switchView(button.dataset.view));document.querySelectorAll("[data-graph-mode]").forEach(button=>button.onclick=()=>{state.graphMode=button.dataset.graphMode;document.querySelectorAll("[data-graph-mode]").forEach(x=>x.classList.toggle("active",x===button));resetGraphView();renderWiki()});$("archRoot").onchange=event=>setArchitectureRoot(event.target.value);$("archBack").onclick=backArchitecture;$("fitArch").onclick=()=>fitArchitecture(true);$("archTlm").onchange=event=>{state.arch.showTlm=event.target.checked;renderArchitecture()};$("archInterfaces").onchange=event=>{state.arch.showInterfaces=event.target.checked;renderArchitecture()};$("resetGraph").onclick=()=>{resetGraphView();renderWiki()};$("globalSearch").oninput=event=>{state.query=event.target.value.trim().toLowerCase();renderCurrent()};$("tlmKind").onchange=event=>{state.tlmKind=event.target.value;renderTlm()};$("runCodeSearch").onclick=runCodeSearch;$("codeSearch").onkeydown=event=>{if(event.key==="Enter")runCodeSearch()};window.addEventListener("resize",()=>{if(state.view==="wiki")resizeGraph();else if(state.view==="architecture")renderArchitecture()})}
...
const ARCH_GAP=18,ARCH_PAD=18,ARCH_HEADER=52,ARCH_LEAF_W=196,ARCH_LEAF_H=78;
...
function archWirePath(wire,index){
  const a=wire.startAnchor,b=wire.endAnchor,offset=(index%5-2)*4;
...
const archRuntime={svg:null,layer:null,wireLayer:null,nodeLayer:null,markLayer:null,context:{svg:null,layer:null,wireLayer:null,nodeLayer:null,markLayer:null},zoom:null,bound:false,motion:null,reducedMotion:window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches||false};
function ensureArchitecture(){
  if(archRuntime.bound)return;
  archRuntime.svg=d3.select("#archGraph");archRuntime.layer=d3.select("#archLayer");archRuntime.wireLayer=d3.select("#archWires");archRuntime.nodeLayer=d3.select("#archNodes");archRuntime.markLayer=d3.select("#archMarks");
  archRuntime.context.svg=d3.select("#archContextGraph");archRuntime.context.layer=d3.select("#archContextLayer");archRuntime.context.wireLayer=d3.select("#archContextWires");archRuntime.context.nodeLayer=d3.select("#archContextNodes");archRuntime.context.markLayer=d3.select("#archContextMarks");
...
function clearArchRuntime(runtime){runtime.wireLayer.selectAll("*").remove();runtime.nodeLayer.selectAll("*").remove();runtime.markLayer.selectAll("*").remove()}
function drawArchBoundary(runtime,scene){
  if(!scene.lane)return;
  const boundary=runtime.nodeLayer.append("g").attr("class","arch-external-boundary");
...
function drawArchitectureScene(runtime,scene,options={}){clearArchRuntime(runtime);drawArchBoundary(runtime,scene);drawArchWires(runtime,scene,options);drawArchNodes(runtime,scene,options)}
