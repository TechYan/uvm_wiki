...
如果 factory create 带有 parent 参数，但目标 class 定义没有进入索引，Architecture 会分别显示每条静态实例证据，不再按相同 type 合并计数；Topology 会显示对应的 `definition missing` class。点击节点会跳到 create 语句。右侧 Related 区域可以按 `creates`、`has_member`、`extends` 等 relation 类型筛选并搜索。优先检查构建日志以及 `metadata.input.unresolved_includes`；这通常表示 package 引入的 VIP 文件没有被找到或超出了 `--src`。
